### NIDA HAFEEZ 
## Hi 👋 I'm Nida Hafeez

Passionate about logic buiding and eager to grow through Problem Solving and real-world projects.
- 🎓 I’m currently learning DSA (Data Structure and Application). Learning Java, C++, OOP, and Data Structure
- 👩‍🎓 Interested in building meaningful projects

---

- **Core Concepts:** OOP, Basic DSA, Problem Solving

## 📌 Features Projects
- 🎮 **Rock-Paper-Scissor(Java)** Console-based game using OOP principles
- 📑 **Learning Management System(Java)** Console-based management system

## 🚀 Goals for 2026

- [x] Build strong foundation in DSA (C++)
- [ ] Complete Graphs topic and finish DSA course
- [ ] Start solving problems on LeetCode / Codeforces
- [ ] Build and push real-world projects
- [ ] Improve GitHub consistency & green contributions
- [ ] Prepare for internships and future tech roles

---

## 🧠 About Me

- 🎓 CS Student passionate about problem solving
- 💻 Currently completing **DSA in C++** — on the final topic: **Graphs!**
- ☕ Strong foundation in **Java OOP**
- 🤖 Interested in AI/ML — building the right foundations first
- 🚀 Aspiring to build useful, real-world applications
- ✨ Looking for internship opportunities in 2026

---

## 🔥 GitHub Streak
  ![GitHub Streak](https://streak-stats.demolab.com?user=nida179&theme=radical)
---

## 🛠️ Skills & Technologies

**Languages:**
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)

**Concepts:**
![OOP](https://img.shields.io/badge/OOP-8A2BE2?style=flat)
![DSA](https://img.shields.io/badge/DSA-20B2AA?style=flat)
![Problem Solving](https://img.shields.io/badge/Problem%20Solving-FF6347?style=flat)

---
## 📨📬 Connect Withe Me
-GitHub: [github.com/nida179] 

-Emai: [nidahafeez31@gmail.com]

